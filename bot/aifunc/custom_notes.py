...
def notesGptCallFlashcards(notes: str, subject: str, numOfCards: str):
	...
	completion = client.chat.completions.create(
	  ...
	  messages=[
	    ...
	    {"role": "user", "content": "Generate " + numOfCards + " flash cards about "+ subject +''' based on the provided text deliminated by triple backticks and organize the front and back in a single JSON format delaminated by the square braces: [ {"id": <card number>, "front": "<front of the card>", "back": "<back of the card>"}, ...]. ```''' + 
	    notes + "```."}
	  ]
	)
